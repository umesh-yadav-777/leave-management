...
function AdminFeatureLeaveRequests() {
  ...
  if (loading) return <div className="p-5 text-center"><h5>Loading Leave Requests...</h5></div>;
  ...
  return (
    <div style={{ padding: '30px', backgroundColor: '#fcfcfd', minHeight: '100vh' }}>
      ...
      <div className="row">
        {requests.length === 0 ? <p className="text-center">No leave requests found.</p> :
        requests.map((req) => (
            ...
            <div className="card border-0 shadow-sm" style={{
              ...
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <h5 className="fw-bold mb-0">{req.user_name}</h5>
                    <small className="text-primary fw-bold">{req.leave_type}</small>
                  </div>
                ...
                <div className="d-flex justify-content-between small text-muted mb-4">
                  <span>Start: <strong>{req.start_date}</strong></span>
                  <span>End: <strong>{req.end_date}</strong></span>
                </div>

                {req.status === 'pending' && (
                  <div className="d-flex gap-2">
                    <button className="btn btn-success flex-grow-1" onClick={() => handleAction(req.id, 'approved')} style={{ borderRadius: '10px' }}>Approve</button>
                    <button className="btn btn-outline-danger flex-grow-1" onClick={() => handleAction(req.id, 'rejected')} style={{ borderRadius: '10px' }}>Reject</button>
                  </div>
                )}
              </div>
            </div>
